...
var mongourl="mongodb+srv://harsh:edureka@cluster0.gkhrd.mongodb.net/Edureka?retryWrites=true&w=majority";//27017 is the default port number of mongourl.
...
app.get('/restaurantList/:mealtype',(req,res) => {
    ...
    }else if(req.query.city&&req.query.sort){
        condition={"type.mealtype":req.params.mealtype,city:req.query.city}
        sort={city:Number(req.query.sort)}
...
MongoClient.connect(mongourl,(err,connection)=>{
  if (err) throw err;
  db=connection.db('Edureka'); //As you can see here if connection is established then we connect it to the database of the Project the db on left hand side is different from connection.(db)
  app.listen(port,(err)=>{
    if (err) throw err;
